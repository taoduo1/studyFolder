...
<h3> 设计和规划基准测试 </h3>
规划基准测试第一步是提出问题并明确目标，然后决定是采用标准的基础测试还是设计专用的测试。

如果是标准的基准测试，应该确认了选择的合适的方案。例如：不要使用TPC-H测试电子商务系统。
因为TPC-H是即席查询和决策支持型应用的基准测试。因此不适合用来测试OLTP系统。

设计专用的测试是很复杂的，往往需要一个迭代的过程，首先需要获得生产数据集的快照，并且该快照很容易还原，以便进行后续的测试。
然后进行单元测试，但是这个单元测试和生产环境上的压力还是区别很大的，这时可以选择一个有代表性的时间段（业务高峰期），记录所有的查询，然后用这些查询进行测试。
...
2. 找出某条语句查询为什么执行不够快
3. 诊断被用户描述成“停顿”、“堆积”或者卡死的某些间歇性疑难故障

<h2> 性能优化简介 </h2>
性能定义为：完成某件任务所需的时间度量。性能即响应时间。
数据库服务器的性能用查询的响应时间来度量，单位为每个查询花费的时间。
优化是：在一定的工作负载下尽可能的降低响应时间。

目标确定了是要降低响应时间，那么首先要理解为什么服务器执行要花那么长时间，然后减去或者消除那些对获得查询结果来说不必要工作。
也就是说，要先搞清时间花在哪里了。

搞清楚时间花在哪就需要合适的测量范围，合适的测量范围是说只测量需要优化的测量范围，有两种比较常见的情况会导致不合适的测量：
1. 在错误的时间启动和停止测量
2. 测量的是聚合后的信息，而不是目标活动本身

例如：一个常见的错误是先看慢查询，然后又去排查整个服务器的情况来判断问题在哪里，如果确认有慢查询，那么就应该测量慢查询而不是测量整个服务器，测量的应该是从慢查询开始到结束的时间，而不是查询之前或之后的时间。

完成一项任务所需的时间可以分为两部分：执行时间和等待时间。
如果要优化任务的执行时间，最好的办法是通过测量定位不同的子任务花费的时间，然后优化去掉一些子任务，降低子任务的执行频率或者提升子任务的效率。
优化等待时间则复杂一些，因为等待有可能是由其他系统间接影响导致，任务之间也可能由于争抢磁盘空间或者CPU资源的影响。


<h2> 通过性能剖析进行优化 </h2>

性能剖析是测量和分析时间花费在哪里的主要方法，性能剖析一般有两个步骤：测量任务所花费的时间，然后对结果进行统计和排序，将重要的任务放在前面。
