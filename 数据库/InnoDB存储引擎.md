...
一. 基本概念
重做日志用来实现事务的持久性，即事务ACID中的D，它是由两部分组成，一是内存中的重做日志缓冲（redo kig buffer），二是重做日志文件（redo log file）

1. InnoDB 通过Force log at commit机制实现事务的持久性。即当事务提交时，必须先将该事务的所有日志写入到重做日志文件惊醒持久化，待事务中的commit操作完成才算完成。这里的日志是指重做日志，在InnoDB中，由两部分组成，即redo log和undo log。 redo 用来保证事务的持久性，undo log来帮助事务回滚即MVCC操作。redo log基本上都是顺序写的，在数据库运行时不需要堆redo log的文件进行读取操作，而undo log是需要进行随机读写的。
...
若一个页产生的重做日志大于512字节，那么需要分割为第一个重做日志块，每块的大小为512字节，若一个页中产生的数量大于512字节，那么需要对其进行分割
