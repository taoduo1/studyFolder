...
![运行时数据区域](F:\work_space\studyFolder\java基础\java虚拟机\assets\运行时数据区域.png)
  ...
  1. <h4>堆(线程共享)</h4>
      ...
      4. 规范规定，java堆可以处于物理上不连续的内存空间中，只要逻辑上是连续的即可
         1. 如果在堆中没有内存完成实例分配，并且堆无法再扩展时，会抛出 **OutOfMemoryError** 异常
  2. <h4>方法区(线程共享)</h4>
...
<h3> （四）垃圾收集算法</h3>
  ...
  **主要不足之处是：将内存分为两块的方法，代价过高，如果存活对象过多会导致复制时间增加，导致效率降低**
  ...
  ![标记-整理算法](F:\work_space\studyFolder\java基础\java虚拟机\assets\标记-整理算法.png)
...
举例：
 ...
 <h3> 字节码指令 </h3>
  ...
  ![图片1](F:\work_space\studyFolder\java基础\java虚拟机\assets\图片1.png)
  <h4> 公有设计和私有实现思想 </h4>
  ...
  ![类的生命周期](F:\work_space\studyFolder\java基础\java虚拟机\assets\类的生命周期.png)
...
![线程、主内存、工作内存的交互关系](F:\work_space\studyFolder\java基础\java虚拟机\assets\线程、主内存、工作内存的交互关系.png)
...
<h3>java 线程的实现</h3>
在jdk1.2之前，是使用用户线程实现的，在1.2中，线程模型替换为基于操作系统原生线程模型实现。
对于sum Jdk来说，win版和linux版都是使用一对一进程实现的，一条java线程对应一条轻量级进程中。
Solaris平台中，由于操作系统的线程特性可以同时支持一对一及多对多，的线程模型，因此在Solaris版的Jdk中，提供了两个专用参数，来明确指定虚拟机的线程模型

<h3>java 线程调度</h3>
线程调度是指系统为线程分配处理器使用权的过程，主要有两种调度方式，分别是：协同式线程调度和抢占式线程调度。
协同式线程调度：线程的执行时间由线程本身控制，线程把自己的工作处理完后，要主动通知系统切换到另一个线程上。
优点是：实现简单，线程操作对线程自己是可知的。
缺点是：执行时间不可控，有可能有一个线程阻塞了，系统就完全阻塞住，有可能导致一个进程坚持不让出cpu，最后导致系统崩溃
抢占式线程调度：每个线程由系统来分配执行时间，线程的切换不由自己来决定。
优点是：线程的执行时间系统可控，不会因一个线程导致系统崩溃，如果某个线程出现问题，还可以直接将线程杀掉。 java使用线程的方式就是抢占式调度。
虽然java线程调度是系统自动完成的，但是可以设置高优先级更容易让系统选择执行（java有10个优先级）。
不过java的线程是通过映射到系统的原生线程上来实现的，所以线程的调度最终还是取决于操作系统，但是由于系统的优先级有可能比java少，就会导致java的优先级在系统中其实是一样的，如图
![](assets/java线程与windows系统线程对比.png)
不过我们也不能太依赖优先级，因为线程优先级有可能被操作系统改变，例如Windows中存在一个“优先级推进器” 它的作用就是发现某个线程非常勤奋，就会越过线程优先级去给这个线程分配CPU执行时间，因此，我们不能通过优先级来准确的判断两个处于ready状态的线程会先执行哪个。
<h3>线程状态转换</h3>
java定义了6种状态，在任意一个时间点，一个线程有且仅有一种状态，这5种分别是：
1. 新建(New)：创建后未启动的线程
2. 运行(Runable)：Runable 包括了操作系统线程状态的running和ready，也就是处于这个状态的线程有可能正在执行，也有可能正在等CPU给它分配执行时间。
3. 无限期等待(Waiting)：处于这个状态的线程不会被Cpu分配执行时间，它们要等待被其它的线程显示的唤醒，以下方法会让线程进入无限期等待
   1. 没有设置Timeout参数的Object.wait()方法
   2. 没有设置Timeout参数的Thread.join()方法
   3. LockSupport.park()方法
4. 限期等待(Timed Waiting)：处于这种状态的线程也不会被分配CPU执行时间，不过无需等待被其他线程显式的唤醒，在一定时间后会被系统自动唤醒，以下方法会让线程进入限期等待状态：
   1. Thread.sleep()方法
   2. 设置了Timeout参数的Object.wait()方法
   3. 设置了Timeout参数的Thread.join()方法
   4. LockSupport.parkNanos()方法
   5. LockSupport.parkUntil()方法
5. 阻塞(Blocked)：线程被阻塞了，阻塞状态与等待状态的区别是：阻塞状态在等待一个排他锁，这个事件将在另外一个线程放弃这个锁的时候发生，而等待状态则是在等待一段时间，或者唤醒动作的发生，在程序等待进入同步区域的时候线程将进入这种状态。
6. 结束(Terminated): 已终止线程的状态，线程已经执行结束

![](assets/线程状态转换关系.png)

<h2>线程安全与锁优化</h2>
<h3>线程安全</h3>
当多个线程访问一个对象时，如果不考虑这些线程在运行环境下的调度和交替执行，也不需要额外的同步，或者在调用方进行任何其他的操作，调用这个对象都可以获得正确的结果，那么这个对象就是线程安全的。

<h3>java中的线程安全</h3>
如果一段代码根本不会与其他线程共享数据，那么从线程安全来看，程序是串行话或是并行执行对它来讲都是完全没有区别的。
我们可以将线程的安全程度从高到底来排序，就可以将各种操作的数据分为以下5类：
1. 不可变  
不可变的对象一定是线程安全的，无论是对象的方法实现还是方法的调用者，都不需要再采取任何的线程安全保障措施。
java语言中，如果共享数据是一个基本数据类型，那么只要在定义时使用final关键字修饰就可以保证它是不可变的。如果共享数据是一个对象，那就需要保证对象的行为不会对其状态产生任何影响，例如将对象所有的属性都声明为final的
2. 绝对线程安全  
绝对的现场安全是：不管运行时环境如何，调用者都不需要进行额外的同步措施。通常付出的代价是很大的
在java API中标注自己是线程安全的类，大多数都不是线程安全的，例如Vector类：
```
    ...
    public static void main(String[] arges) {
        while (true) {
            ...
            Thread printThread = new Thread(() -> {
                //打印列表中的数，如果 这里 vector 是安全的，这里会锁
...
```
尽管这里使用到的 Vector 的get()、remove() 方法都是同步的，但是在多线程环境中，如果不在方法调用端做额外处理的话，使用代码仍然是不安全的 会抛出java.lang.ArrayIndexOutOfBoundsException

如果我们这里不希望抛出 异常，代码应修改为
...
3. 相对线程安全
   ...
   相对线程安全就是我们通常意义上讲的线程安全，它需要保证这个对象单独的操作是线程安全的，我们在调用的时候不需要做额外的保障措施，但是对于一些特定的顺序调用，就可能需要一些手段来保证调用的正确性，在java语言中，大部分线程安全都属于这个类型，例如：Vector、HashTable、Collections的synchronizedCollection()方法包装的集合等。
4. 线程兼容
    
    线程兼容是指对象本身不是线程安全的，但是可以通过在调用端使用正确的同步手段来保证对象在并发环境中可以安全的使用，我们平常说的一个类不是线程安全的，绝大多数指的是这个情况  
    java中绝大多数类属于这个情况，与前面Vector和HashTable对应的是ArrayList和HashMap
5. 线程对立
    
    线程对立是指无论调用端采取任何措施，都无法在并发环境下使用的代码，由于java语言天生就具备多线程特性，线程对立这种代码是很少出现的，而且通常都是有害的，应当尽量避免，例如  
    Thread的suspend()方法和resume()方法，如果两个线程同时持有一个对象，一个尝试去中断线程，一个尝试去恢复线程，如果并发进行的话，无论调用时会否进行了同步，目标线程都是存在死锁风险的，如果suspend()中断的线程就是即将要执行resume()的线程，那就肯定要死锁了，也正由于这个原因，suspend()和resume()方法已经被JDk声明废弃。  
    常见的线程对立操作还有System.setIn(),System.setOut()和System.runFinalizersOnExit()等。

<h3>线程安全的实现方法</h3>
1. 互斥同步  
互斥同步是常见的并发正确性保障手段，同步时指在多个线程并发访问共享数据时，保证共享数据同一时刻只能被一个线程使用，而互斥是实现同步的一种手段，临界区、互斥量和信号量都是主要的互斥方式。  
在java中，最基本的同步互斥手段就是synchronized关键字，synchronized关键字在编译后，会在同步块的前后分别生成 monitorenter和monitorexit这两个字节指令，这两个字节指令都需要一个reference类型的参数来指明要锁定和解锁的对象，synchronized明确指定了对象参数，那锁的就是这个对象参数，如果没有指定，就会根据synchronized修饰的是实例方法还是类方法，去取对应的对象实例或class对象来作为锁对象。
在虚拟机规范中，monitorenter和monitorexit的行为描述中，有两点是需要特别注意的，首先synchronized对同一线程是可重入的，不会自己把自己锁死，其次，同步块在已进入的线程执行完之前，会阻塞后面其他的线程，因为java的线程实现是依靠操作系统的原生线程实现的，如果阻塞或者唤醒一个线程，都需要操作系统来帮忙完成，这个开销是异常巨大的。对于一些代码简单的代码块，线程状态转换的时间会比执行代码的时间还长，所以synchronized操作是一个重量级操作（java1.6之后也对synchronized进行了一些优化）。
除了synchronized，还可以使用java.util.concurrent包中的重入锁(ReentrantLock)。在用法上，ReentrantLock和synchronized相似，也都具备可重入特性，区别就是一个是API层面的互斥锁，另一个表现为原生语法层面的互斥锁。
相比于synchronized，ReentrantLock增加了一些高级功能：等待可中断、可实现公平锁以及锁可以绑定多个条件
   1. 等待中断是指当持有锁的线程长期不释放的时候，正在等待的线程可以选择放弃等待，改为处理其他的事情，可中断特性对处理执行时间非常长的同步块很有帮助
   2. 公平锁是指多个线程在等待同一个锁时，必须按照申请锁的时间顺序来依次获得锁，而非公平锁则不保证这一点，在锁被释放时，任何一个等待锁的线程都有机会获得锁。synchronized是非公平的，ReentranLock默认情况下也是非公平的
   3. 锁绑定多个条件是指一个ReentranLock对象可以同时绑定多个Condition对象，而在synchronized中，锁对象的wait()和notify()或notifyAll()方法可以实现一个隐含的条件，如果要和多于一个的条件进行关联的时候，就要多添加一个锁，而ReentranLock不需要，只需要多次调用newCondition()方法即可。
在后续的吞吐量测试中，1.6之后和synchronized和ReentranLock的性能基本持平，而且虚拟机在未来的性能改进中也会更偏向synchronized。 所以提倡在synchronized能满足需求的情况下优先使用synchronized
2. 非阻塞同步
互斥同步带来的问题就是进行线程阻塞和唤醒带来的性能问题，因此这种同步也成为阻塞同步。互斥同步属于一种悲观的并发策略（认为只要不去做正确的同步措施，就肯定会出问题，无论是否出现竞争，都要去加锁）。
非阻塞同步是硬件指令集发展后，基于冲突检测的乐观并发策略，先进行操作，如果没有其他线程共享数据，那操作就成功，否则采取其他的补偿措施（常见措施就是不断重试直至成功为止）。
非阻塞同步如果互斥同步就失去意义了，只能依靠硬件确保一个从语义上看起来需要多次的操作通过一条cpu指令完成。
这类常用的指令有：
   ...
   4. 比较并交换(CAS)
   5. 加载链接/条件存储(LL/SC)
其中，前3条是20世纪之前就已经存在于大多数指令集中的处理器命令，后面两条是现代处理器新增的，而且这两条的功能和目的类似。
CAS需要有3个操作数，分别是内存位置，旧操作数，新操作数。CAS指令执行时，当且仅内存位置上的旧值符合传入的旧值时，才会去用新值更新内存位置上的值，否则就不执行更新，这个过程是一个原子操作。
3. 无同步方案
要保证线程安全，并不一定必须同步，如果一个方法本来就不共享数据，那它自然也不需要去保证线程安全。
   1. 可重入代码，就是代码不涉及其他线程参与，在执行一半之后去执行其他线程代码，回来之后依然能正确执行的代码
   2. 线程本地存储，数据放在本地线程中，例如java的 ThreadLocal

<h3>锁优化</h3>
<h4>自旋锁与自适应自旋</h4>
为了让线程等待，让线程进入空循环就是自旋锁
自旋锁等待虽然避免了线程切换的开销，但它是要占用处理器时间的，如果锁被占用的时间短，自旋等待效果就好，如果锁的时间长那自旋只会白白消耗处理器资源。
自旋锁由JDK1.4引入，1.6后默认开启。之前的自旋锁是默认10次，1.6后改进为自适应自旋，自旋时间不是固定的，而是由前一次在同一个锁上的自旋时间及锁的拥有者状态来决定
<h4>锁消除</h4>
锁消除是指虚拟机即时编译器在运行中，对一些代码上要求同步，但是被检测到不可能存在共享数据竞争数据的锁进行消除，锁消除的主要判定依据来源是逃逸分析的数据支持。
如果判断在一段代码中，堆上的所有数据都不会逃逸出去从而被其他线程访问到，那就可以把它们当做栈上数据对待，认为它们是线程私有的，同步加锁自然就无需进行。
主要原因是因为很多锁不是程序员写的，而是java自己带的， 比如：
```
...
```
append()方法 是加了 synchronized的方法，在所锁消除后，这个锁就会被消除
<h4>锁粗化</h4>
一般来说，同步块推荐将锁作用范围尽可能小，但是如果是上面的连续的append方法，可以将锁范围扩大，只需执行一次加锁操作即可
<h4>轻量级锁</h4>
HotSpot虚拟机的对象头分为两部分，第一部分存储对象自身的运行时数据，如哈希码、GC分代年龄等（这部分称为“Mark Word”，它是实现偏向锁的关键）。另一部分用于存储指向方法区对象类型数据的指针，如果是数组对象的话，还有一个额外的部分用于存储数组长度。
对象头信息是与自身定义的数据无关的额外储存成本，考虑到虚拟机的空间效率Mark Word 被设计成一个非固定的数据结构以便在极小的空间内存储更多的信息，它会根据对象的状态复用自己的存储空间。

![](assets/虚拟机对象头.png)
在代码进入同步块的时候，如果此同步对象没有被锁定（锁标志位为 01），虚拟机首先将在当前栈帧中建立一个名为锁记录（Lock Record）的空间，用于存储锁对象目前的Mark world 拷贝。  
然后，虚拟机将使用CAS操作尝试将对象的Mark world 更新为指向Lock Record 的指针，如果这个操作成功了，那么这个线程就拥有了该对象的锁，并且对象的Mark world 的锁标志位将转为 00，表示这个对象处于轻量级锁 锁定状态。  
如果这个操作失败了，虚拟机会检查对象的Mark world是否指向当前线程的栈帧，如果只说明当前线程已拥有了这个对象的锁，那就可以直接进入同步块继续执行，否则说明这个对象已经被其他线程抢占了。如果有两个以上的线程争用同一个锁，那轻量级锁就不再有效，要膨胀为重量级锁，锁状态置为 10，Mark world 中存储的就是指向重量级锁的指针，后面等待锁的线程也要进入阻塞状态。
轻量级锁的解锁过程也是通过CAS操作来进行的，如果对象的Mark world仍然指向着线程的锁记录，那就用CAS操作把对象当前的Mark world和线程中复制的对象头替换回来，如果替换成功，整个过程同步就完成了，如果替换失败，说明有其他线程尝试获取过该锁，就要在释放锁的同时，将其他被挂起的线程唤醒。
轻量级锁的依据是：对于绝大部分的锁，在整个同步周期内都是不存在竞争的，这是靠经验总结的，没有竞争的时候，轻量级锁使用CAS操作，避免了互斥量的开销，但是如果存在竞争的话，除了互斥量的开销，还增加了CAS操作，会比重量级锁更慢。

<h4>偏向锁</h4>
偏向锁也是JDK1.6之后引入的一项锁优化，它的目的是消除数据在无竞争状态下的同步原语，如果说轻量级锁是在无竞争的时候使用CAS去消除同步使用的互斥量，那偏向锁就是在无竞争的时候把整个同步都消除掉，CAS操作也不做。
假设当前虚拟机启用了偏向锁后，当对象第一次被线程获取的时候，虚拟机会将对象头中的标志位设置为 “01” 即偏向模式，同时使用CAS操作把获取到这个锁的线程ID记录在对象的对象头中，如果CAS操作成功，持有偏向锁的线程以后每次进入这个锁相关的同步块的时候，虚拟机都可以不用再进行任何同步操作了，但是当有另外一个线程去尝试获取这个锁的时候，偏向锁就宣告结束，根据对象目前是否处于被锁定的状态，撤销偏向锁，转为轻量级锁。
![](assets/偏向锁转为轻量级锁.png)
偏向锁可以提高有同步但无竞争的程序性能，它与轻量级锁一样，如果同步块总是被多个线程同时访问，那偏向锁就是多余的，反而比加锁更慢。
